...
def resume_reader(personal_data, result, links, worksheet, choices, row):

    worksheet.write(row, 0, personal_data.get('name'))
    worksheet.write(row, 1, personal_data.get('email'))
    worksheet.write(row, 2, personal_data.get('phone'))
    col = 3 
...
if __name__ == "__main__":
    ...
    for i in range(len(choices)):
        ...
        worksheet.write(0, i+3, heading)

    # ITERATING OVER EACH RESUME, AND WRITING THE RESULT TO EXCEL FILE
    row = 1
    for resume in resumes:
        resume_path = os.path.join(dir_path, resume)
        result, links = doc_parser(resume_path)
        personal_data = resumeparse.read_file(resume_path)
        resume_reader(personal_data, result, links, worksheet, choices, row)
        row += 1
